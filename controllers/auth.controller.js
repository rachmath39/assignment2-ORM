...
exports.register = async (req, res, next) => {
    ...
    try {
        ...
        res.status(201).json({message: "Success creating new user", name: user.name, username: user.username, email: user.email, role: user.role, PhoneNumber: user.PhoneNumber, address: user.address});
    } catch (error) {
        next(error);
    }
};
...
try {
    ...
    const token = sign(payload, process.env.JWT_SECRET, {expiresIn: "5m"});
